---
nav\_title: Cartero y Muestra Solicita article\_title: Solicitudes de cartero y muestra page\_order: 3 Descripción: "Este artículo de referencia cubre nuestra nueva colección Braze Postman, qué es, cómo configurar y usar la colección, así como cómo editar y enviar sus solicitudes." page\_type: reference

---

# Cartero y muestras de solicitudes

> Braze le permite generar solicitudes de API de ejemplo para todos los endpoints a través de nuestra colección. Este artículo de referencia cubre la colección Braze Postman, qué es, cómo configurar y usar la colección, así como cómo editar y enviar solicitudes.

## ¿Qué es Postman?

Postman es una herramienta de edición visual gratuita para crear y probar solicitudes de API. A diferencia de otros métodos para interactuar con API (por ejemplo, usando cURL), Postman le permite editar fácilmente solicitudes de API, ver información de encabezados y mucho más. Postman tiene la capacidad de guardar colecciones o bibliotecas de solicitudes de API prefabricadas de muestra. Para facilitar a nuestros clientes la puesta en marcha de nuestra API REST, hemos creado una colección con ejemplos prefabricados para todos nuestros endpoints API. Este es un EXCELENTE TEXTO NUEVO AQUÍ QUE DEBE SER DETECTADO TAMBIÉN!!!

Vea nuestra colección de carteros haciendo clic en **Ejecutar en cartero** en nuestros [documentos de cartero](https://documenter.getpostman.com/view/4689407/SVYrsdsG?version=latest#intro) para comenzar.

## Usando la colección Braze Postman

Si tiene una cuenta de Postman (puede descargar los tipos de macOS, Windows y Linux desde el [sitio web Postman][1]), puede abrir la documentación de Postman en su propia aplicación Postman haciendo clic en el botón naranja **Ejecutar en Postman**. A continuación, puede [crear un entorno](#setting-up-your-postman-environment), o utilizar nuestro entorno de API Braze REST como plantilla, y editar las solicitudes `POST` y `GET` disponibles para satisfacer sus propias necesidades.
